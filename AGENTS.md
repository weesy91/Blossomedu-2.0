...
## 📅 2026-01-28: 시험 모드 고도화 및 버그 수정

### 1. 단어 시험(Class Log Test) 동기화 오류 수정 (Critical)
- **증상**: 수업 일지 작성 화면에서 '시험(Projector)' 실행 시, 프로젝터 화면과 선생님 채점 화면의 단어 목록/순서가 불일치하는 치명적 오류.
- **원인**: 두 화면이 각각 독립적으로 서버에 `shuffle: true`를 요청하여 서로 다른 무작위 목록을 수신함.
- **해결**:
  - '선생님 더보기 > 시험' 기능과 동일한 **SharedPreference 동기화 패턴** 적용.
  - 다이얼로그에서 단어를 미리 생성(Fetch & Shuffle)한 후 로컬 저장소에 저장.
  - 프로젝터는 저장된 데이터를 로드하고, 채점 화면은 생성된 데이터를 그대로 전달받아 사용함으로써 **100% 일치 보장**.

### 2. 단어 시험 범위 오류 수정
- **증상**: 단원(Unit) Dropdown으로 시험 범위를 설정했을 때, 내부 `range` 문자열이 비어있어 시험이 제대로 실행되지 않거나 엉뚱한 단어가 출제됨.
- **해결**: `startUnit`과 `endUnit`이 선택된 경우, 자동으로 `1-3` 형태의 범위 문자열을 생성하여 API에 전달하도록 로직 개선.

### 3. 연습 모드 (Practice Mode) 구현
- **목표**: 학생들이 패널티나 시간 제한 없이 단어를 학습할 수 있는 '연습 모드' 추가.
- **구현 내용**:
  - **오답 패널티 제거**: 틀려도 7초 대기 없이 바로 진행 가능.
  - **건너뛰기 허용**: 모르는 단어는 빈 칸으로 제출하여 스킵 가능.
  - **UI/UX**: 타이머 숨김, 상단 바 '연습 모드'로 변경.
  - **진입점 추가**: 과제 상세 화면 및 스노우볼 설정에 '연습 모드' 버튼 추가.

### 4. 기타 버그 수정 및 개선
- **단어 과제 노출 기간 연장**: `origin_log_date`가 아닌 `due_date`를 기준으로 7일간 노출되도록 수정하여, 마감일 이후에도 충분히 보강 기한을 제공.
- **텍스트 수정**: 앱 내 "실험" 오타를 "시험"으로 일괄 수정 (`study_screen.dart`).

---



### 1. 학생 계정 관리 통계 (Student Stats Banner)
- **목표**: 원장/관리자가 분원별 활성 수강생 현황을 즉시 파악할 수 있도록 함.
- **구현 내용**:
  - **Banner**: '학생 계정 관리' 화면 상단에 총원 및 분원별 인원 배지 표시.
  - **권한 관리**:
    - **Superuser(Admin)**: 전체 지점 합계 및 모든 지점별 내역 조회.
    - **Principal/Vice**: 본인이 소속된 지점의 인원수만 조회 (`views_api.py` 권한 로직 개선).
    - **Teacher**: 통계 배너 숨김 처리.

### 2. 단어 검색 고도화 (Word Search Improvements)
- **목표**: 단어 검색 시 정확도 향상 및 결과 표시 방식 개선.
- **구현 내용**:
  - **정확도 순 정렬**: DB 검색 시 단어 길이 순으로 정렬하여 정확한 매칭(`cost`)이 유사 결과(`costume`)보다 상단에 오도록 개선.
  - **품사(POS) 표준화**: Google API 결과를 앱 표준 약어(`n.`, `v.`, `a.`, `ad.` 등)로 변환하여 표시 (`[verb] -> v.`).
  - **오타 교정 최적화**: 이미 정확한 단어(`cost`)인 경우 과도한 자동 교정(`-> costume`) 방지 로직 추가.
  - **Backend Parser**: `n. 뜻 / v. 뜻` 형태의 다중 품사를 정확히 파싱/저장하도록 `services.py` 및 `utils.py` 구분자 처리(`/`) 개선.

### 3. 선생님 앱 메뉴 조정
- **학생 메뉴 임시 비활성화**: 선생님 홈 하단 '학생' 탭 클릭 시 "준비중" 다이얼로그 표시 (추후 대체 메뉴 도입 예정).

---

## 📅 2026-01-24: 학생 로그 검색 기능 및 단어장 파싱 규칙 고도화

### 1. 학생 로그 통합 검색 (Log Search)
- **목표**: 선생님이 학생의 수업일지, 과제, 단어시험 기록을 한눈에 조회하고 검색할 수 있는 기능 제공.
- **구현 내용**:
  - **Backend**: `StudentLogSearchView` API 구현 (`ClassLog`, `AssignmentTask`, `TestResult` 통합).
  - **Frontend**: `TeacherMoreScreen` 내 '학생 로그 검색' 메뉴 추가 및 `StudentLogSearchScreen` 구현.
  - **기능**:
    - 학생 이름 자동완성 검색 (Autocomplete).
    - 날짜 범위 및 유형(Log, Assignment, Test) 필터링.
    - 리스트 아이템 클릭 시 상세 다이얼로그(진도, 코멘트 확인) 및 해당 리뷰 화면으로 이동.
  - **Router**: `/teacher/management/log-search` 경로 등록 (기존 학생 ID 경로와 충돌 방지).

### 2. 수업일지 상세 정보 확장
- **문제**: 기존 로그 요약 정보만으로는 진도나 과제 내역 확인이 어려움.
- **해결**:
  - **API 개선**: `ClassLog`의 모든 `entries`(진도), `score`(평가), `comment`(코멘트) 및 `generated_assignments`(관련 과제)를 포함하여 응답.
  - **UI 개선**: 상세 다이얼로그에서 진도 리스트와 출제된 과제 목록을 구조화하여 표시, 과제 클릭 시 바로가기 제공.

### 3. 단어장 업로드 파싱 고도화
- **목표**: 단어장 CSV 업로드 시 사용자의 다양한 입력 패턴을 자동으로 정제.
- **구현 내용**:
  - **Regex 강화**: `vocab/services.py`의 `clean_text` 함수 업데이트.
  - **규칙 적용**:
    - 세미콜론(`;`) → 쉼표(`,`) 자동 변환.
    - 괄호 `( )` 및 내부 텍스트 자동 제거.
    - 대괄호 확장: `지배[좌우]하다` → `지배하다, 좌우하다` 로 자동 분리.
  - **기존 데이터 정리**:
    - `manage.py cleanup_vocab` 커맨드 추가.
    - 관리자 페이지(`WordAdmin`)에 '선택한 단어 뜻 정제' 액션 추가로 소급 적용 지원.
...
### 🔴 우선순위: 높음 (핵심 기능)
- [x] **학생 로그 검색** - 수업/과제/단어 기록을 모아서 검색/조회 가능한 페이지 (선생님 더보기 메뉴)
- [x] **담당강사 관리** - 원장: 당일 분원 전체 현황(출결/일지작성여부) 파악. 부원장: 담당 강사의 학생 현황 파악. 수업일지 기록 열람 기능 연동.
- [x] **성적표 기능** - 학생별 수업일지, 단어 기록 등을 종합하여 성적표 생성 및 발송
